...
def validation(args, model, test_pts, logger, batch_n, epoch, verbose=False):
    ...
    for pos_a, pos_b, neg_a, neg_b in test_pts:
        ...
        if pos_a.num_graphs != 32 or neg_a.num_graphs != 32:
            print(pos_a.num_graphs, neg_a.num_graphs)
            pdb.set_trace()
    ...
    f1_score_macro = f1_score(labels.cpu(),pred.cpu(),average='macro')
    f1_score_weighted = f1_score(labels.cpu(),pred.cpu(),average='weighted')
    ...
    print("Validation. Epoch {}. Acc: {:.4f}. "
        ...
        "TN: {}. FP: {}. FN: {}. TP: {}.\n     F1-score: {:.4f}. F1-macro: {:.4f}. F1-weighted: {:.4f}".format(epoch,
            acc, prec, recall, auroc, avg_prec,
            tn, fp, fn, tp,F1_score,f1_score_macro,f1_score_weighted))
    ...
    if not args.test:
        ...
        logger.add_scalar("F1score/test", F1_score, batch_n)
        logger.add_scalar("F1Macro/test", f1_score_macro, batch_n)
