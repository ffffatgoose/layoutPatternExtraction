...
def train(args, model, logger,step_num): #in_queue, out_queue
    """Train the order embedding model.
    ...
    while not done:
        ...
        for batch_target, batch_neg_target, batch_neg_query in zip(*loaders):
            ...
            pos_a, pos_b, neg_a, neg_b = data_source.gen_batch(batch_target,
                batch_neg_target, batch_neg_query, True,N= idx_i)
            idx_i += 1
            ...
            if (step_num + idx_i) % 100 == 0:
                print("batch step:",step_num + idx_i,train_loss,train_acc)
        done = True
...
def train_loop(args):
    ...
    for batch_target, batch_neg_target, batch_neg_query in zip(*loaders):
        pos_a, pos_b, neg_a, neg_b = data_source.gen_batch(batch_target,
            batch_neg_target, batch_neg_query, False,N=idx_i)
