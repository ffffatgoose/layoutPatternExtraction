        ...
        for simulation_n in tqdm(range(self.n_trials //
            ...
            while frontier and len(neigh) < self.max_size:
                ...
                cand_embs = self.model.emb_model(utils.batch_nx_graphs(
                    cand_neighs, anchors=anchors if self.node_anchored else None, mode='Decoder'))
...
class GreedySearchAgent(SearchAgent):
    ...
    def init_search(self):
        ...
        for trial in range(self.n_trials):
            ...
            # for tmp_cand_node in frontier_origin:
            #     tmp_cand_neigh.append(graph.subgraph(neigh + [tmp_cand_node]))
            #     if self.node_anchored:
            #         tmp_cand_anchors.append(neigh[0])
            # tmp_cands_emb = self.model.emb_model(
            #     utils.batch_nx_graphs(tmp_cand_neigh, anchors=tmp_cand_anchors if self.node_anchored else None))
            # choosing_result = self.model.predict((current_neigh_emb, tmp_cands_emb))
            ...
            #     for index in nozero_index_cand:
            #         frontier.append(frontier_origin[index])
            #
            #     visited = set([start_node])
            #     beams.append([(0, neigh, frontier, visited, graph_idx)])
        ...
        for beam_set in tqdm(self.beam_sets):
            ...
            for _, neigh, frontier, visited, graph_idx in beam_set:
                ...
                for cand_node in frontier:
                    ...
                    # print("node_anchored:",self.node_anchored)
                ...
                for cand_node, cand_emb in zip(frontier, cand_embs):
                    ...
                    for emb_batch in self.embs:
                        ...
                        if self.model_type == "order":
                            tmp_result = self.model.predict((emb_batch.to(utils.get_device()), cand_emb))
                            tmp_result1 = self.model.clf_model(tmp_result.unsqueeze(1))
                            tmp_result2 = torch.argmax(tmp_result1, axis=1)
                            tmp_result3 = torch.sum(tmp_result2).item()
                            # if len(neigh) > 6:
                            #     pdb.set_trace()
                            score -= tmp_result3
                    ...
                    for tmp_cand_node in new_frontier_origin:
                        tmp_cand_neigh.append(graph.subgraph(neigh + [cand_node,tmp_cand_node]))
                        if self.node_anchored:
                            tmp_cand_anchors.append(neigh[0])
                    tmp_cands_emb = self.model.emb_model(utils.batch_nx_graphs(tmp_cand_neigh,anchors=tmp_cand_anchors if self.node_anchored else None, mode='Decoder'))
                    choosing_result = self.model.predict((cand_emb, tmp_cands_emb))
                    choosing_result1 = self.model.clf_model(choosing_result.unsqueeze(1))
                    choosing_result2 = torch.argmax(choosing_result1, axis=1)
                    choosing_result3 = torch.sum(choosing_result2).item()
                    ...
                    if choosing_result3 >= 1:
                        ...
                        nozero_index_cand = torch.nonzero(choosing_result2, as_tuple=False).view(1, -1).tolist()[0]
                        for index in nozero_index_cand:
                            new_frontier.append(new_frontier_origin[index])
            ...
            for score, neigh, frontier, visited, graph_idx in new_beams[:1]:
                ...
                if self.analyze and len(neigh) >= 3:
                    emb = self.model.emb_model(utils.batch_nx_graphs(
                        ...
                        else None, mode='Decoder')).squeeze(0)
