...
FEATURE_AUGMENT, FEATURE_AUGMENT_DIMS = ["node_attrs"], [8]
...
class FeatureAugment(nn.Module):
    def __init__(self):
        ...
        def node_attrs_fun(graph, feature_dim):
            graph.node_attrs = torch.zeros((len(graph.G.nodes), 8))
            for i, v in enumerate(graph.G.nodes):
                feat = (graph.G.nodes[v]["x"] if "x" in
                                                 graph.G.nodes[v] else np.zeros(8))
                graph.node_attrs[i, :8] = torch.tensor(
                    feat, dtype=torch.float)
            return graph
