...
FEATURE_AUGMENT, FEATURE_AUGMENT_DIMS = ["node_attrs"], [6]
...
class FeatureAugment(nn.Module):
    def __init__(self):
        ...
        def degree_fun(graph, feature_dim): #出度入度？
        ...
        def node_attrs_fun(graph, feature_dim):
            graph.node_attrs = torch.zeros((len(graph.G.nodes), feature_dim))
            for i, v in enumerate(graph.G.nodes):
                feat = (graph.G.nodes[v]["x"] if "x" in
                    graph.G.nodes[v] else np.zeros(NODE_FEATURE_DIM))
                graph.node_attrs[i,:NODE_FEATURE_DIM] = torch.tensor(
                    feat, dtype=torch.float)
            return graph

        # def node_label(graph,)
...
class Preprocess(nn.Module):
    ...
    def forward(self, batch):
        # pdb.set_trace()#look for the detailed of batch.node_feature
        if AUGMENT_METHOD == 'concat':
